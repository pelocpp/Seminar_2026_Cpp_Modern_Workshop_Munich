...
static void print(const int& n)  // free function
{
    int m = n;
...
static void lambda_01_modern()
{
    ...
    Printer myPrinter{};  // default c'tor
...
{
    auto lambdaOne = [](void) -> void { std::println("One"); };
    auto lambdaTwo = [](void) -> void { std::println("Two"); };
    auto lambdaThree = [](int n) -> void { std::println("Two"); };

    std::function<  void(int) > func{ lambdaThree };
